...
try:
    while True:      
        ...
        distance = TMF_get_distance()													#NEW: (assuming TMF on the right)
        ...
        if (vals in junction_sensor_values) and (current_node in box_nodes_list) and (distance < threshold):		#NEW: trying 1 distance sensor
            ...
            current_node = nodedirections[current_node][4]							#set destination to minor of current                                                           
            count = 1
            graph = rt.RouteGraph(nodedirections) 
            router = rt.Router(graph)
